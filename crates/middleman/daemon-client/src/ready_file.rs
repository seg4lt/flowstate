...
//   1. Serde format of `ReadyFileContent` + `TransportAddressInfo` —
...
/// Contents of the daemon's ready file. Mirrors
/// `zenui_daemon_core::ready_file::ReadyFileContent` — the duplication
/// is deliberate, see the module-level comment above.
...
/// Address info for one transport entry. Duplicated from
/// `daemon-core::transport::TransportAddressInfo` — keep them in sync.
/// The `#[serde(tag = "kind", rename_all = "kebab-case")]` matches the
/// daemon's serialization exactly.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
mod tests {
    ...
    fn ready_file_roundtrip() {
        ...
        let back: ReadyFileContent = serde_json::from_str(&json).unwrap();
