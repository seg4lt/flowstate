...
Library crates consumed by apps under `../apps/`. Organized into two
layers:

- **[`core/`](./core/README.md)** — the domain. The agent runtime,
  session orchestration, persistence, the shared wire protocol, and
  the provider adapters that talk to external AI processes.
  Transport-agnostic — these crates don't know whether they're running
  inside a daemon, a CLI, or a test harness.
