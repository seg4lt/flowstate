...
function readInitial(): TerminalState {
  // Persisting `defaultDockOpen` is safe: TerminalDock's auto-spawn
  // effect is gated on a false→true transition of `dockOpen` or a
  // project switch, so a restored "open" on launch won't resurrect
  // a throwaway shell — the dock just appears empty until the user
  // hits the "+" button or toggles it closed.
...
function terminalReducer(
  ...
  switch (action.type) {
    ...
    case "close_tab": {
      ...
      const projects = new Map(state.projects);
      let defaultDockOpen = state.defaultDockOpen;
      let dockOpenBySession = state.dockOpenBySession;
      if (tabs.length === 0) {
        projects.delete(action.projectKey);
        // If no project has any tabs left, fold the dock everywhere.
        // The auto-spawn guard in TerminalDock would already block a
        // resurrect, but folding gives the user the "close all = dock
        // hides" gesture from the pre-per-session model. We clear
        // both the global default and any per-session overrides so
        // no stale "open" override resurrects the empty dock when
        // the user navigates between threads.
        if (projects.size === 0) {
      ...
      return { ...state, projects, defaultDockOpen, dockOpenBySession };
    }
    case "set_active_tab": {
      const current = state.projects.get(action.projectKey);
      if (!current) return state;
