...
#[command(
    name = "flowzen server",
    about = "The Flowzen daemon: owns the runtime and serves clients over HTTP/WS.",
    version
)]
struct Cli {
    #[command(subcommand)]
...
pub fn run(argv: Vec<String>) -> i32 {
    let mut parser_argv: Vec<String> = vec!["flowzen server".to_string()];
    ...
    if let Err(err) = result {
        eprintln!("flowzen server error: {err:?}");
        return 1;
    }
    0
}
...
fn run_start(args: StartArgs) -> Result<()> {
    ...
    std::fs::create_dir_all(project_root.join("threads")).ok();
...
fn spawn_detached(project_root: &PathBuf, args: &StartArgs, _bind: SocketAddr) -> Result<()> {
    ...
    let child = cmd
        .spawn()
        .context("failed to spawn detached flowzen server")?;
    eprintln!(
        "flowzen server spawned (pid={}), waiting for ready file; logs: {}",
        child.id(),
        log_path.display()
    );
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            bail!(
                "timeout waiting for flowzen server ready file; see {}",
                log_path.display()
            );
        }
...
fn run_stop(args: StopArgs) -> Result<()> {
    ...
    let content = match ready.read()? {
        Some(c) => c,
        None => {
            println!("flowzen server: no ready file for {}", project_root.display());
            return Ok(());
        }
    };
    ...
    let http_base = content
        ...
        .ok_or_else(|| {
            anyhow!(
                "daemon has no HTTP transport; transports: {:?}",
    ...
    let response = ureq::post(&shutdown_url)
        ...
        .call();
    match response {
        Ok(_) => println!("flowzen server: shutdown request accepted"),
        Err(ureq::Error::Status(code, _)) if code == 204 || code == 200 => {
            println!("flowzen server: shutdown request accepted");
        }
    ...
    while Instant::now() < deadline {
        if ready.read()?.is_none() {
            println!("flowzen server: exited cleanly");
            return Ok(());
        }
        std::thread::sleep(Duration::from_millis(50));
    }
    bail!("flowzen server did not delete its ready file within 10s")
}
...
fn run_status(args: StatusArgs) -> Result<()> {
    ...
    println!("flowzen server running");
...
fn log_file_path() -> Result<PathBuf> {
    ...
    let dir = base.join("flowzen").join("logs");
    Ok(dir.join("flowzen-server.log"))
}
